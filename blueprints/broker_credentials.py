...
def get_credentials():
    ...
    try:
        ...
        creds = get_user_broker_credentials(username) if username else None

        broker_api_key = creds.get("broker_api_key") if creds else ""
        broker_api_secret = creds.get("broker_api_secret") if creds else ""
        broker_api_key_market = creds.get("broker_api_key_market") if creds else ""
        broker_api_secret_market = creds.get("broker_api_secret_market") if creds else ""
        redirect_url = creds.get("redirect_url") if creds else ""
        broker_api_environment = (
            (creds.get("broker_api_environment") if creds else "") or "sandbox"
        )
        ...
        current_broker = get_broker_from_redirect_url(redirect_url) if redirect_url else ""
        ...
        return jsonify(
            {
                ...
                "data": {
                    ...
                    "broker_api_environment": broker_api_environment,
...
def update_credentials():
    ...
    try:
        ...
        if request.is_json:
            ...
            broker_api_environment = data.get("broker_api_environment", "").strip().lower()
            ...
            broker_api_environment = request.form.get("broker_api_environment", "").strip().lower()
        ...
        if broker_api_environment and broker_api_environment not in {"sandbox", "production"}:
            return jsonify(
                {
                    "status": "error",
                    "message": "Invalid broker API environment. Use 'sandbox' or 'production'.",
                }
            ), 400
        ...
        upsert_user_broker_credentials(
            ...
            broker_api_environment=broker_api_environment if broker_api_environment else None,
            broker_api_key=broker_api_key if broker_api_key else None,
            broker_api_secret=broker_api_secret if broker_api_secret else None,
            broker_api_key_market=broker_api_key_market if broker_api_key_market else None,
            broker_api_secret_market=broker_api_secret_market if broker_api_secret_market else None,
        ...
        if broker_api_environment:
            updated_fields.append("BROKER_API_ENVIRONMENT")
        ...
        # Update global settings in .env if provided
        ...
        logger.info(f"Updated broker credentials for user {username}: {', '.join(updated_fields)}")
        return jsonify(
            {
                "status": "success",
