...
from broker.tastytrade.api.nxtradstream import NxtradStream
from database.token_db import get_br_symbol, get_oa_symbol, get_symbol, get_token
from broker.tastytrade.api.urls import get_tastytrade_base_host, get_tastytrade_base_url
...
class TastyTradeWebSocket:
    def __init__(self):
        """Initialize WebSocket connection using official TastyTrade SDK"""
        self.nx_stream = None
        ...
        self.nxtrad_host = get_tastytrade_base_host()
    ...
    def connect(self, auth_token):
        """Connect to TastyTrade WebSocket using official SDK"""
        try:
            ...
            # Format the auth token exactly as per TastyTrade requirements
            ...
            logger.debug("Connecting to TastyTrade WebSocket using official SDK")
            ...
            if self.connected:
                logger.debug("Successfully connected to TastyTrade WebSocket")
                return True
            else:
                logger.error("Failed to connect to TastyTrade WebSocket within timeout")
                return False
    ...
    def subscribe_depth(self, symbol, exchange, token):
        ...
        try:
            ...
            # Format token as per TastyTrade requirement
...
class BrokerData:
    def __init__(self, auth_token):
        """Initialize TastyTrade data handler with authentication token"""
        self.auth_token = auth_token
        self.ws = TastyTradeWebSocket()

        # Map supported timeframe formats for TastyTrade
        # Note: TastyTrade only supports 1m, 5m, and 30m intervals
        self.timeframe_map = {
            "1m": "1m",  # 1 minute
            "5m": "5m",  # 5 minutes
            "30m": "30m",  # 30 minutes
    ...
    def connect_websocket(self):
        ...
        try:
            ...
            logger.info("Connecting to TastyTrade WebSocket...")
            success = self.ws.connect(self.auth_token)

            if success and self.ws.connected:
                logger.info("Successfully connected to TastyTrade WebSocket")
                return True
            else:
    ...
    def _format_quote(self, quote_data: dict, symbol: str, exchange: str) -> dict:
        """Format quote data from TastyTrade to OpenAlgo standard format"""
        try:
    ...
    def get_quotes(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            # Subscribe to quotes - format as per TastyTrade requirements
    ...
    def get_multiquotes(self, symbols: list) -> list:
        ...
        TastyTrade WebSocket supports up to 3,000 instruments per connection
        ...
        """
        try:
            # TastyTrade WebSocket can handle up to 3000 instruments
    ...
    def _process_multiquotes_batch(self, symbols: list) -> list:
        ...
        for item in symbols:
            ...
            # Format as per TastyTrade requirements: token_exchange
            symbol_key = f"{token}_{exchange}"
        ...
        with self.ws.lock:
            for symbol_key, info in symbol_map.items():
                # Try different key formats that TastyTrade WebSocket might use
                possible_keys = [
    ...
    def _format_depth(self, depth_data: dict, symbol: str, exchange: str) -> dict:
        """Format depth data from TastyTrade to OpenAlgo standard format"""
        try:
    ...
    def get_history(
        ...
        try:
            ...
            # Map exchange to TastyTrade format
            exchange_map = {
                "NSE": "NSE",
                "BSE": "BSE",
                "NFO": "NFO",
            ...
            # Get symbol in TastyTrade format
            token_str = get_symbol(token, exchange)

            # Map interval to TastyTrade format
    ...
    def _get_historical_data(
        ...
        """Fetch historical OHLC data from TastyTrade REST API"""
        try:
            # API endpoint
