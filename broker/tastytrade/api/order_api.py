...
def get_api_response(endpoint, auth, method="GET", data=None, params=None):
    """
    Make API request to tastytrade API with proper authentication.
...
def get_order_book(auth):
    """
    Get list of orders placed using tastytrade API.
    ...
    try:
        ...
        base_url = get_tastytrade_base_url()
        response = client.get(
            f"{base_url}/accounts/{accountId}/orders",
            headers=headers,
                    ...
                    transformed_order = {
                        ...
                        "data": {
                            ...
                            if openalgo_symbol
                                ...
                                "sym"
                            ],  # Fallback to tastytrade symbol if OpenAlgo not found
...
def get_trade_book(auth):
    """
    Get list of trades using tastytrade API.
    ...
    try:
        ...
        # Make API request
        logger.info("get_trade_book - Making request to tastytrade API")
        base_url = get_tastytrade_base_url()
        response = client.get(
            f"{base_url}/v2/api/oms/trades",
            headers=headers,
            params={"symDetails": "true"},
        )
...
def get_positions(auth):
    """
    Get list of positions using tastytrade API.
    ...
    try:
        ...
        base_url = get_tastytrade_base_url()
        response = client.get(
            f"{base_url}/v2/api/oms/positions",
            headers=headers,
            params={"symDetails": "true"},
            timeout=10,
        )
        ...
        logger.info(
            f"Raw positions response from tastytrade API: {json.dumps(response_data, indent=2)}"
        )
...
def get_holdings(auth):
    """
    Get list of holdings using tastytrade API.
    ...
    try:
        ...
        logger.debug("Fetching holdings from tastytrade API")
...
def place_order_api(data, auth):
    """
    Place an order using tastytrade API.
    ...
    try:
        ...
        try:
            client = get_httpx_client()
            base_url = get_tastytrade_base_url()
            url = f"{base_url}/v2/oms/place-order"
...
def place_smartorder_api(data, auth):
    """
    Place a smart order using tastytrade API.
...
def close_all_positions(current_api_key, auth):
    """
    Close all open positions using tastytrade API.
...
def cancel_order(orderid, auth):
    """
    Cancel an order using tastytrade API.
...
def cancel_all_orders_api(data, auth):
    """
    Cancel all open orders using tastytrade API.
...
def modify_order(data, auth):
    """
    Modify an order using tastytrade API.
    ...
    try:
        ...
        # Transform data to tastytrade format
        try:
