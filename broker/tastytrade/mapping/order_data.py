...
from broker.tastytrade.mapping.transform_data import reverse_map_product_type
...
def map_order_data(order_data):
    ...
    Parameters:
    - order_data: tastytrade API response containing order information
...
def map_trade_data(trade_data):
    ...
    Args:
        trade_data: tastytrade API response containing trade information
    ...
    # Check if it's a tastytrade API response
    if not isinstance(trade_data, dict) or "s" not in trade_data or trade_data.get("s") != "ok":
        # Not a tastytrade API response - log at debug level instead of warning to avoid unnecessary warnings
        logger.debug("map_trade_data - Not a tastytrade API response format")
        return []
...
def transform_tradebook_data(trades):
    ...
    # Check if raw tastytrade API response
    if isinstance(trades, dict) and "s" in trades and trades.get("s") == "ok" and "d" in trades:
        logger.debug("transform_tradebook_data - Processing raw tastytrade API response")
        trades = trades.get("d", [])
...
def map_position_data(position_data):
    """
    Map tastytrade position data to a standardized format.
