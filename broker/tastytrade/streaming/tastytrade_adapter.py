...
from broker.tastytrade.api.urls import get_tastytrade_streamer_host
from .tastytrade_mapping import tastytradeCapabilityRegistry, tastytradeExchangeMapper


class tastytradeWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """tastytrade-specific implementation of the WebSocket adapter"""

    def __init__(self):
        super().__init__()
        self.logger = logging.getLogger("tastytrade_websocket")
        self.ws_client = None
        self.user_id = None
        self.broker_name = "tastytrade"
    ...
    def initialize(
        ...
        """
        Initialize connection with tastytrade WebSocket API

        Args:
            broker_name: Name of the broker (always 'tastytrade' in this case)
        ...
        if not auth_data:
            ...
            # For tastytrade, the access token is used for both API and WebSocket
            ...
            # Get API key from environment for tastytrade
            api_key = os.getenv("BROKER_API_SECRET", "")

            # Format token for tastytrade WebSocket (api_key:access_token)
            if api_key and ":" not in auth_token:
                ws_token = f"{api_key}:{auth_token}"
            else:
                ws_token = auth_token

            ws_url = get_tastytrade_streamer_host()
        else:
            # Use provided tokens
            ...
            ws_url = auth_data.get("ws_url") or get_tastytrade_streamer_host()

            if not auth_token:
                self.logger.error("Missing required authentication data")
                raise ValueError("Missing required authentication data")

            # Get API key from environment or auth_data for tastytrade
    ...
    def connect(self) -> None:
        """Establish connection to tastytrade WebSocket"""
    ...
    def _connect_with_retry(self) -> None:
        """Connect to tastytrade WebSocket with retry logic"""
        while self.running and self.reconnect_attempts < self.max_reconnect_attempts:
            try:
                self.logger.info(
                    f"Connecting to tastytrade WebSocket (attempt {self.reconnect_attempts + 1})"
                )
    ...
    def subscribe(
        ...
        """
        Subscribe to market data with tastytrade-specific implementation

        Args:
            symbol: Trading symbol (e.g., 'RELIANCE')
            exchange: Exchange code (e.g., 'NSE', 'BSE', 'NFO')
            mode: Subscription mode - 1:LTP, 2:Quote, 3:Depth
            depth_level: Market depth level (5 for tastytrade)
        ...
        if mode == 3:  # Depth mode
            if not tastytradeCapabilityRegistry.is_depth_level_supported(exchange, depth_level):
                # If requested depth is not supported, use the highest available
                actual_depth = tastytradeCapabilityRegistry.get_fallback_depth_level(
    ...
    def unsubscribe(self, symbol: str, exchange: str, mode: int = 2) -> dict[str, Any]:
        ...
        if self.connected and self.ws_client:
            try:
                # tastytrade unsubscribes from all tokens for a given type
                if mode in [1, 2]:
    ...
    def _on_connection_event(self, ws, message) -> None:
        ...
        if status == "connected":
            self.logger.info("Connected to tastytrade WebSocket")
            self.connected = True
            ...
            self.logger.error(f"tastytrade WebSocket error: {message.get('reason')}")

        elif status == "closed":
            self.logger.info(f"tastytrade WebSocket connection closed: {message.get('reason')}")
    ...
    def _on_data(self, ws, message) -> None:
        ...
        try:
            ...
            self.logger.debug(f"RAW tastytrade DATA: Type={type(message)}, Data={message}")
            ...
            if msg_type == "auth":
                ...
                if auth_status == 1:
                    self.logger.info("Successfully authenticated with tastytrade WebSocket")
                else:
                    self.logger.error(f"Authentication failed with tastytrade WebSocket: {message}")
                return
    ...
    def _extract_depth_data(self, depth_list) -> list[dict[str, Any]]:
        """
        Extract depth data from tastytrade's message format
