...
class tastytradeExchangeMapper:
    """Maps exchange codes between OpenAlgo and tastytrade formats"""

    # Exchange mappings based on tastytrade's SEG_INFO
    EXCHANGE_MAP = {
        "NSE": "NSE",
        "BSE": "BSE",
        "NFO": "NFO",
    ...
    # Exchange segment IDs used in tastytrade WebSocket
    EXCHANGE_SEGMENTS = {
        1: "NSE",
        2: "BSE",
        3: "NFO",
    ...
    def get_exchange_segment(cls, exchange_code: str) -> int:
        """
        Get tastytrade exchange segment ID from exchange code

        Args:
            exchange_code: Exchange code (e.g., 'NSE', 'BSE')

        Returns:
            int: Exchange segment ID for tastytrade
        """
    ...
    def get_exchange_from_segment(cls, segment_id: int) -> str:
        """
        Get exchange code from tastytrade segment ID

        Args:
            segment_id: tastytrade exchange segment ID
...
class tastytradeCapabilityRegistry:
    """Registry for tastytrade-specific capabilities and limitations"""

    # Depth level support by exchange
    # tastytrade supports 5-level depth for all exchanges
    DEPTH_CAPABILITIES = {
